...
def main(cfg: DictConfig) -> None:
    ...
    all_doc_embeddings = torch.from_numpy(np.load(embedding_dv_path)).float()
    all_expert_ids = torch.from_numpy(np.load(expert_ids_path)).long()

    X_train, X_temp, y_train, y_temp = train_test_split(
        all_doc_embeddings, all_expert_ids, test_size=0.2, stratify=all_expert_ids, random_state=42
    )
