...
def visualize_umap(query_embedding, top_doc_embeddings, top_doc_ids, query_id, output_dir, experts_used, relevants, use_adapters=True):
    ...
    handles = [
        mlines.Line2D([], [], color="yellow", marker="o",
                      ...
                      linestyle="None", markersize=14, label="query"),
    ...
    filename = (
        f"umap_query_{query_id}_2D_experts.png"
        if use_adapters
        else f"umap_query_{query_id}_2D_NO_experts.png"
    )
...
def visualize_deepview(
    ...
    save_path = os.path.join(
        output_dir,
        f"deepview_query_{query_id}_experts{num_experts}_{dataset_name}.png",
    )
